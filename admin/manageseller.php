...
<?php
  ...
  if(isset($_GET['del']))
  {
    ...
    echo "<script> location.replace('manageseller.php') </script>";
  }
  if(isset($_GET['verify']))
  {
    
    $sql1="UPDATE LOGIN SET ACTIVE=1 WHERE LOGIN_ID=".$_GET['verify']."";
    $result=mysqli_query($con,$sql1);
    echo "<script> location.replace('verifiedseller.php') </script>";
    ...
    <section class="content">
      ...
      <div class="col-xs-12">
        ...
        <?php
          ...
          $query = "SELECT * FROM LOGIN WHERE ACTIVE='0' AND ROLE='2' ";
          /*$query="SELECT LOGIN.EMAIL_ID,LOGIN.LOGIN_ID,LOGIN.PHONE_NO,LOGIN.PROFILE_PIC,SELLER.NAME FROM LOGIN INNER JOIN SELLER ON LOGIN.LOGIN_ID=SELLER.LOGIN_ID WHERE LOGIN.ACTIVE=0 AND LOGIN.ROLE=2";*/
        ...
        ?>
          ...
          <td> 
          <!-- <a class="btn btn-info btn-xs" href="viewseller.php?sid=<?php echo $value2['LOGIN_ID'];?> " 
          onclick="return confirm('sure to view detail?');">VIEW</a> &nbsp;&nbsp;
           --><a class="btn btn-warning btn-xs" href="?verify=<?php echo $value2['LOGIN_ID'];?>" 
          onclick="return confirm('sure to verify seller?');">VERIFY</a> &nbsp;&nbsp;
