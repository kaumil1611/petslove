<?php
	...
	else
	{
		...
		$qry = "SELECT LOGIN_ID FROM LOGIN WHERE EMAIL_ID='$req' OR PHONE_NO='$req'";
		$result = mysqli_query($con,$qry);
		$value = mysqli_fetch_array($result);
		$id = $value['LOGIN_ID'];

		$sql = "SELECT OTP FROM OTP WHERE OTP_ID = (SELECT MAX(OTP_ID) FROM OTP WHERE LOGIN_ID='$id' )";
...
<?php
	  ...
	  if(isset($_POST['submit']))
	  {
		...
		if($otp==$otp_num)
		{
			if($passw3==$passw2)
			{
			  $update = "UPDATE LOGIN SET PASSWORD='$passw1' WHERE LOGIN_ID='$id'";
